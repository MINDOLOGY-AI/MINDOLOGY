...
def main():
    ...
    for dataset in DATASETS:
        ...
        assert len(unknown) >= need, f"{dataset}: only {len(unknown)} unknown of {len(rows)}, need {need}"
        print(f"{dataset}: {len(unknown):,} unknown of {len(rows):,} ({100 * (1 - len(unknown) / len(rows)):.1f}% known)")
        splits = {
            "A": unknown[:N_SPLIT],
        ...
        for name, split_rows in splits.items():
            out = out_dir / f"{dataset}_{name}.jsonl"
