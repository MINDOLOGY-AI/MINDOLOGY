# picks: per-unit example windows for labeling (see synapse/interp/DOC.md, dynamic activation analysis).
# gather_picks runs a hooked model over a tokenized chunk dataset and streams every batch through a PicksTracker,
# which keeps, per hook, per unit, over the whole run, vectorized over units on device:
...
def gather_picks(hooked, source_bin, source_shape, hook_names, n_chunks, out_dir, batch_chunks):
    ...
    tracker = PicksTracker({name: hooked.acts[name].shape[-1] for name in hook_names}, ctx_len, n_chunks, device)
